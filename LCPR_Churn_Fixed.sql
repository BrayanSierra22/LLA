...
SELECT  DATE_TRUNC('MONTH',dt) AS fix_s_dim_month
        ...
        ,IF(fix_s_att_account IN (SELECT DISTINCT sub_acct_no_sbb FROM BOM_subsidized_customers),1,0) AS fix_b_fla_subsidized
...
SELECT  DATE_TRUNC('MONTH',(dt - interval '1' day)) AS fix_s_dim_month
        ...
        ,IF(fix_s_att_account IN (SELECT DISTINCT sub_acct_no_sbb FROM EOM_subsidized_customers),1,0) AS fix_e_fla_subsidized
...
SELECT  fix_s_dim_month
        ...
        ,fix_e_fla_subsidized
        ,fix_e_att_BillCode
        ,IF(fix_s_fla_churn_type = '2. Fixed Involuntary Churner','6.Null last day',IF(fix_s_att_account IN (SELECT distinct account_id FROM transfers) AND fix_s_fla_main_movement IN('4.New Customer','5.Come Back to Life'),'7.Transfer Adds',fix_s_fla_main_movement)) AS fix_s_fla_main_movement
        ...
        ,CASE   WHEN fix_s_fla_main_movement = '6.Null last day' AND fix_s_fla_churn_type IS NULL THEN '3. Fixed Transfer'
        ...
        ,CASE   WHEN fix_s_fla_MainMovement = '6.Null last day' AND fix_s_fla_churn_type IS NULL THEN 'Transfer'
