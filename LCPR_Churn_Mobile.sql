...
-----------------------------------------------------------------------------------------
-- CREATE TABLE IF NOT EXISTS "db_stage_dev"."lcpr_mob_apr2023_adj2" AS
...
SELECT  subsrptn_id AS account
        ...
        ,(date(dt_2) + interval '1' month - interval '1' day) AS mob_b_dim_date
        ,CASE 
            WHEN subsrptn_id not in (SELECT subsrptn_id FROM missing_accounts) THEN DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day)) 
            WHEN subsrptn_id in (SELECT subsrptn_id FROM missing_accounts) THEN DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day - interval '1' month))
            END AS mob_b_mes_tenure_days
        ,date(activation_dt) AS mob_b_dim_max_start
        ,CASE   
        --- Early tenure
        WHEN subsrptn_id not in (SELECT subsrptn_id FROM missing_accounts) and DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day)) <= 180 THEN 'Early-Tenure'
        WHEN subsrptn_id in (SELECT subsrptn_id FROM missing_accounts) and DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day - interval '1' month)) <= 180 THEN 'Early-Tenure'
        --- Mid tenure
        WHEN subsrptn_id not in (SELECT subsrptn_id FROM missing_accounts) and DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day)) <= 360 THEN 'Mid-Tenure'
        WHEN subsrptn_id in (SELECT subsrptn_id FROM missing_accounts) and DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day - interval '1' month)) <= 360 THEN 'Mid-Tenure'
        --- Late tenure
        WHEN subsrptn_id not in (SELECT subsrptn_id FROM missing_accounts) and DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day)) >  360 THEN 'Late-Tenure'
...
FROM (
    ...
    AND rgn_nm <> 'VI'
    AND subsrptn_sts = 'A')
    UNION ALL (SELECT * FROM missing_accounts WHERE row_nm = 1 and dt_2 = (SELECT input_month FROM parameters)    
    --> Flags utilizadas para clasificar a los usuarios residenciales. Input de Juan C. Vega
    AND cust_sts = 'O'
    AND acct_type_cd = 'I'
    AND rgn_nm <> 'VI'
...
SELECT  subsrptn_id as account
        ,cust_id AS parent_account
        ,(date(dt_2) + interval '1' month - interval '1' day) AS mob_e_dim_date
        ,DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day)) AS mob_e_mes_tenure_days
        ,date(activation_dt) AS mob_e_dim_max_start
        ,CASE   WHEN DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day)) <= 180 THEN 'Early-Tenure'
                WHEN DATE_DIFF('day',date(activation_dt),(date(dt_2) + interval '1' month - interval '1' day)) <= 360 THEN 'Mid-Tenure'        
...
SELECT  (SELECT input_month FROM parameters) AS mob_s_dim_month
        ...
        else 0 end AS mob_b_fla_active
        ,IF(B.account IS NOT NULL,1,0) AS mob_e_fla_active
        ,mob_b_dim_date
...
SELECT  A.mob_s_att_ParentAccount
        ...
        ,sum(A.mob_b_fla_active) as bom_active
        ,sum(A.mob_e_fla_active) as eom_active
        ,CASE WHEN sum(A.mob_b_fla_active) = 0 THEN Null else (B.MRC_per_parent/sum(A.mob_b_fla_active)) end as BOM_MRC_per_subs
        ,CASE WHEN sum(A.mob_e_fla_active) = 0 THEN Null else (C.MRC_per_parent/sum(A.mob_e_fla_active)) end as EOM_MRC_per_subs
...
LEFT JOIN (SELECT * FROM MRC_ext_calculus WHERE dt = (SELECT case when input_month < date('2023-04-01') then input_month - interval '1' month else input_month end as input_month FROM parameters)) C
...
FROM full_flags
WHERE 
    mob_b_fla_active + mob_e_fla_active >= 1
...
--     count(distinct case when mob_e_fla_active = 1 then mob_s_att_account else null end) as BOM_RGUs
-- FROM full_flags
-- WHERE
--     mob_b_fla_active + mob_e_fla_active >= 1
